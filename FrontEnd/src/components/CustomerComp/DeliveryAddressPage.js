...
const DeliveryAddressPage = () => {
  ...
  const validateExpirationDate = () => {
    ...
    const [month, year] = paymentData.expirationDate.split('/').map((str) => parseInt(str, 10));
    const expirationDate = new Date(year, month - 1, 1); // Set the day to 1 to avoid issues with different days

    return expirationDate >= currentDate;
  };

  const isCardNumberValid = () => {
    // Card number should be a 16-digit number and not start with zero
    return /^[1-9]\d{15}$/.test(paymentData.cardNumber);
  };


  const isCVVValid = () => {
    // You can add your CVV validation logic here
    // For simplicity, I'll check if it's a 3-digit number and not 000
    return /^\d{3}$/.test(paymentData.cvv) && paymentData.cvv !== '000';
  };
